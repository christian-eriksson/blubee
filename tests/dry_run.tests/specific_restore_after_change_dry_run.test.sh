...
test_dir="$(pwd)"
backup_json="$test_dir/specific_restore_after_change_dry_run.backup.json"

# GIVEN a test backup json
name_one="specific-one"
name_two="specific-two"
destination="$test_dir/specific_restore_after_change_dry_run.copy"
source_root="$test_dir/specific_restore_after_change_dry_run.result"
json=$(cat << EOM
{
...
config_path="$test_dir/specific_restore_after_change_dry_run.config"
restore_backup_copy_path="$test_dir/specific_restore_after_change_dry_run.restore_backup"
...
./blubee -b "$backup_json" -c "$config_path" -n "$name_one" dry restore

# THEN blubee ran without crashing
test_results="$?"

# AND the backuped directory retains its latest changes
test_results="$test_results $(assert_dirs_equal "$source_root" "$test_dir/specific_restore_after_change_dry_run.expected/root")"

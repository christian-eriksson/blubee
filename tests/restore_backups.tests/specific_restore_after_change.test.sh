...
test_dir="$(pwd)"
backup_json="$test_dir/specific_restore_after_change.backup.json"

# GIVEN a test backup json
name_one="multi-one"
name_two="multi-two"
destination="$test_dir/specific_restore_after_change.copy"
source_root="$test_dir/specific_restore_after_change.result"
json=$(cat << EOM
{
...
config_path="$test_dir/specific_restore_after_change.config"
restore_backup_copy_path="$test_dir/specific_restore_after_change.restore_backup"
...
./blubee -b "$backup_json" -c "$config_path" -n "$name_one" restore

# THEN the restored source directory has the expected files, content and structure
test_results=$(assert_dirs_equal "$source_root" "$test_dir/specific_restore_after_change.expected")

echo "specific_restore_after_change.test.sh\nRESULTS:"

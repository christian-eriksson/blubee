...
script_name="remote_destination_restore_w_custom_port.test.sh"
relative_dir="${0%/*}"
cd $relative_dir
test_dir="$(pwd)"
cd ..
...
# THEN blubee prepends the remote user, host to the destination
...
# THEN blubee uses the port when creating dirs
remote_calls=$(echo "$output" | grep -e "ssh.*-p$port.*$user@$host.*mkdir " | wc -l)
test_results="$test_results $(assert_greater_than $remote_calls 0)"

# THEN blubee sets custom port to the rsync command
remote_calls=$(echo "$output" | grep -e "rsync.*--rsh=ssh.*-p $port" | wc -l)
test_results="$test_results $(assert_equal_numbers $remote_calls 4)"

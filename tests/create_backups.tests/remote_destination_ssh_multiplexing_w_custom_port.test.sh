...
script_name="remote_destination_ssh_multiplexing_w_custom_port.test.sh"
relative_dir="${0%/*}"
cd $relative_dir
test_dir="$(pwd)"
cd ..
...
backup_json="$test_dir/remote_destination_ssh_multiplexing_port.backup.json"
backup_dir="$test_dir/remote_destination_ssh_multiplexing_port.backup"
root="$test_dir/test_files_root"
root_copy="$test_dir/test_files_root_remote_destination_ssh_multiplexing_port.copy"
cp -r $root $root_copy
...
# AND all three SSH multiplexing options are present together with custom port
all_ssh_opts=$(echo "$output" | grep -e "rsync.*-o ControlMaster=auto.*-o ControlPath=/tmp/ssh-%r@%h:%p.*-o ControlPersist=10m.*-p $port" | wc -l)

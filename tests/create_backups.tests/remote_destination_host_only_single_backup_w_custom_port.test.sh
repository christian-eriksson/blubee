...
script_name="remote_destination_host_only_single_backup_w_custom_port.test.sh"
relative_dir="${0%/*}"
cd $relative_dir
test_dir="$(pwd)"
cd ..
...
remote_calls=$(echo "$output" | grep -e "ssh.*-p$port.*$host.*mkdir.*\"$backup_dir/$name.*\"" | wc -l)
